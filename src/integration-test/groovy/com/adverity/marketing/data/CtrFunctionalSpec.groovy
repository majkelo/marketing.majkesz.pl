...
@Integration
class CtrFunctionalSpec extends CommonIntegrationSpec {
	...
	void "Test the index action"() {
		...
		then: "The response is correct"
			...
			response.body().data.first().clicks == dailyStats.clicks
			response.body().data.first().impressions == dailyStats.impressions
			response.body().data.first().ctr == dailyStats.CTR
	}
	...
	void "Test the index action with filtering by campaign"() {
		...
		then: "The response is correct"
			...
			response.body().data.first().clicks == 11
			response.body().data.first().impressions == 200
			response.body().data.first().ctr == 11 / 200
	}
	...
	void "Test the index action with filtering by startDate"() {
		...
		then: "The response is correct"
			...
			response.body().data.first().clicks == 12
			response.body().data.first().impressions == 200
			response.body().data.first().ctr == 12 / 200
	}
	...
	void "Test the index action with filtering by endDate"() {
		...
		then: "The response is correct"
			...
			response.body().data.first().clicks == 1
			response.body().data.first().impressions == 100
			response.body().data.first().ctr == 1 / 100
	}
	...
	void "Test the index action with filtering by startDate and endDate"() {
		...
		then: "The response is correct"
			...
			response.body().data.first().clicks == 2
			response.body().data.first().impressions == 100
			response.body().data.first().ctr == 2 / 100
	}
	...
	void "Test the index action with count"() {
		...
		then: "The response is correct"
			...
			response.body().data.clicksCount == 26
			response.body().data.ctr == 26 / 600
	}
	...
	void "Test the index action with countPerDay and startDate"() {
		...
		then: "The response is correct"
			...
			response.body().data.first().clicksCount == 13
			response.body().data.first().ctr == 13 / 300
	}
	...
	void "Test the show action correctly renders an instance"() {
		...
		then: "The response is correct"
			...
			response.body().data.clicks == dailyStats.clicks
			response.body().data.impressions == dailyStats.impressions
			response.body().data.ctr == dailyStats.CTR
	}
}
